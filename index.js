...
async function handler(req) {
  try {
    ...
    if (!WORKER_LIST[handler_js]) {
      try {
        WORKER_LIST[handler_js] = { resolve: [] }
        var js_code = "";
        if (FILE_MODE) {
          try {
        ...
        if (WORKER_LIST[handler_js]?.resolve) {
          for (let resolve of WORKER_LIST[handler_js].resolve) {
            try {
              resolve()
            } catch (e) { }
          }
          delete WORKER_LIST[handler_js].resolve
        }
        if (!WORKER_LIST[handler_js]?.url) delete WORKER_LIST[handler_js];
      }
